...
chrome.runtime.onMessage.addListener(async (request, sender, sendResponse) => {
  ...
  if (request.type === 'FETCH_AUDIO') {
    ...
    const selectedLanguage = request.language || 'da';

    console.log("selected language: " + selectedLanguage);

    const transeleos_endpoint = `https:/<your ip address here>/store_translated_audio?url=${url}&language=${selectedLanguage}`;

    try {
      fetch(transeleos_endpoint, {
        mode: 'cors',
        method: 'GET',
        timeout: 60000,
      })
