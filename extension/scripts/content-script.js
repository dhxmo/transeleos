(async () => {
    ...
    const openTranseleosEventHandler = async (event) => {
        ...
        if (isPopupVisible) {
            ...
            if (popupMenu) {
                ...
                popupMenu.innerHTML = `
                <div id="transeleos-notification" style="display: none;"></div>
                    ...
                    try {
                        chrome.storage.local.get('audioState', async (result) => {
                            ...
                            if (audioState &&
                                ...
                                try {
                                    console.log("lang right now", selectedLanguage);
    ...
    const hideAudioInfo = () => {
        const notification = document.getElementById('transeleos-notification');

        // Apply some styling
        notification.style.display = 'block';
        notification.style.backgroundColor = 'rgba(255, 0, 0, 0.5)'; // Red background for visibility
        notification.style.color = '#fff'; // White text color
        notification.style.padding = '10px'; // Add some padding
        notification.style.borderRadius = '5px'; // Rounded corners

        notification.innerHTML = 'This usually takes about 5 minutes. Please be patient';
        notification.style.display = 'block';
